...
export default function DateInput({ id, value, onChange, className = "", placeholder = "" }: DateInputProps) {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (
      ...
      (displayValue[currentCursorPosition] === "/" || displayValue[currentCursorPosition - 1] === "/")
    ) {
      // Se estiver deletando uma barra, remover também o dígito anterior
      const newValue =
        displayValue.substring(0, currentCursorPosition - 1) + displayValue.substring(currentCursorPosition + 1)
    ...
    if (cleanValue.length <= 10) {
      ...
      if (cleanValue.length === 2 && !cleanValue.includes("/") && currentCursorPosition === 2) {
        formattedValue = `${cleanValue}/`
        newCursorPosition += 1
      } else if (cleanValue.length === 5 && cleanValue.indexOf("/", 3) === -1 && currentCursorPosition === 5) {
        formattedValue = `${cleanValue}/`
        newCursorPosition += 1
      }
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === "Backspace") {
      ...
      if (displayValue[cursorPos - 1] === "/" && cursorPos > 0) {
        ...
        const newValue = displayValue.substring(0, cursorPos - 2) + displayValue.substring(cursorPos)
