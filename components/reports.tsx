...
export default function Reports({ transactions }: ReportsProps) {
  ...
  const sortTransactions = (transactions: Conta[]) => {
    return [...transactions].sort((a, b) => {
      ...
      switch (sortColumn) {
        ...
        case "description":
          return direction * a.descricao.localeCompare(b.descricao);
        case "type":
          return direction * a.tipo.localeCompare(b.tipo);
        case "amount":
  ...
  const formatValueWithSignAndColor = (
    doc: jsPDF,
    value: number,
    x: number,
    y: number
  ...
  const exportYearlyToExcel = () => {
    ...
    const exportData = sortedYearlyTransactions.map((t) => ({
      "Data Emissão": formatDate(t.data_emissao),
    ...
    const balance = yearlyTransactions.reduce(
      (acc, t) => acc + (t.tipo === "A Receber" ? t.valor : -t.valor),
      0
    );
    exportData.unshift({
      "Data Emissão": "",
  ...
  const exportYearlyToPDF = () => {
    ...
    const incomeTotal = yearlyTransactions
    ...
    doc.setTextColor(239, 68, 68); // Vermelho (#ef4444)
    const expenseTotal = yearlyTransactions
    ...
    const transactionsByMonth: { [key: string]: Conta[] } = {};
      ...
      .forEach((monthIndex) => {
        ...
        if (monthTransactions.length > 0) {
          ...
          const tableRows = monthTransactions.map((t) => [
            formatDate(t.data_emissao),
            formatDate(t.date),
            t.descricao,
            t.tipo,
            t.tipo === "A Receber"
              ? {
                  content: `+ ${formatCurrency(t.valor)}`,
          ...
          doc.autoTable({
            ...
            didDrawPage: (data: { pageCount: number }) => {
              ...
              if (data.pageCount > 1) {
                ...
                doc.setFont("helvetica", "normal");
                doc.text(
                  `Relatório Anual - ${yearFilter} - ${monthName}`,
                  margin,
                  10
                );
              }
  ...
  const exportMonthlyToExcel = () => {
    ...
    XLSX.writeFile(
      workbook,
      `relatorio-mensal-${monthName}-${yearFilter}.xlsx`
    );
  };
  ...
  const exportMonthlyToPDF = () => {
    ...
    const transactionsByDay: { [key: string]: Conta[] } = {};
  ...
  const exportCustomToExcel = () => {
    ...
    const balance = dateRangeTransactions.reduce(
      (acc, t) => acc + (t.tipo === "A Receber" ? t.valor : -t.valor),
      0
    );
    exportData.unshift({
    ...
    exportData.unshift({
      "Data Emissão": "",
      "Data Vencimento": "",
      Descrição: `Relatório Personalizado - ${formatDate(
        startDate
      )} a ${formatDate(endDate)}`,
      Tipo: "",
      Valor: "",
    });
    exportData.unshift({
              ...
              <Card>
                ...
                <CardContent>
                  {(() => {
                    const balance = yearlyTransactions.reduce(
                      (acc, t) =>
                        acc + (t.tipo === "A Receber" ? t.valor : -t.valor),
                      0
                    );
                    return (
                      <div
          ...
          <TabsContent value="monthly" className="space-y-4">
            ...
            <div className="space-y-4">
              ...
              <div className="h-60 sm:h-80 w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart
                    data={prepareMonthlyDetailChartData()}
                    margin={{ top: 20, right: 30, left: 20, bottom: 5 }}
                  >
                    <CartesianGrid strokeDasharray="3 3" />
                    <XAxis dataKey="name" />
                    ...
                    <Bar dataKey="valor" name="Valor">
                      {prepareMonthlyDetailChartData().map((entry, index) => (
                        <Cell
                          key={`cell-${index}`}
                          fill={
                            entry.name === "A Receber" ? "#10b981" : "#ef4444"
                          }
                        />
                      ))}
                    </Bar>
              ...
              {Object.entries(
                sortedMonthlyTransactions.reduce<Record<string, Conta[]>>(
                  (acc, transaction) => {
                    const day = new Date(transaction.date)
                      .getDate()
                      .toString()
